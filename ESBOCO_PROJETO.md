...
6. **Compatibilidade com o ADK 2.x**: `google-adk>=2.9.1,<3` e `mcp>=2.2,<3` (o servidor MCP usa a API 2.x; os extras `google-adk[mcp]`/`[all]` fixam `mcp<2`). Sem overrides de execução de agente, sem eventos anexados na mão e sem `BaseException` engolida (`monitoring/test_compatibilidade_adk.py`). A memória atende `context.add_memory` do ADK, e o diagnóstico aponta banco de sessões no schema legado v0 com o comando `adk migrate session`.
